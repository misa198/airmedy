...
<template>
  <div class="w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 transition-colors"
    :class="[disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer', checked
      ? variant === 'outlined'
        ? 'border-primary'
        : 'border-primary bg-primary'
      : 'border-foreground/20 hover:border-foreground/40']" @click="!disabled && $emit('update:checked', !checked)">
