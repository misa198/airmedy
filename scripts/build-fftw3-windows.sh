#!/usr/bin/env bash
# Build a static FFTW3F (single precision) library for Windows (amd64 + arm64).
# Works on Linux (cross-compile) and Windows/MSYS2 (native).
# Output: internal/infra/audio/fftw3_libs/windows/{amd64,arm64}/libfftw3f.a
...
PKGCONFIG_BASE="${REPO_ROOT}/internal/infra/audio/fftw3_libs/pkgconfig/windows"
...
build_arch() {
    ...
    echo "==> Building FFTW3F ${FFTW_VERSION} for Windows ${ARCH}..."
    rm -rf "${WORK_DIR}"
    ...
    ./configure --host="${HOST_TRIPLE}" --prefix="${WORK_DIR}/install" \
        --disable-shared --enable-static --enable-single --disable-fortran --disable-doc >/dev/null
    ...
    mkdir -p "${OUT_DIR}"
    cp "${LIB}" "${OUT_DIR}/libfftw3f.a"
    echo "    copied libfftw3f.a -> fftw3_libs/windows/${ARCH}/"
    write_pkgconfig "${ARCH}"
}
