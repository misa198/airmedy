#!/usr/bin/env bash
# Build a static FFTW3F (single precision) library for macOS (arm64 + x86_64).
# Output: internal/infra/audio/fftw3_libs/darwin/{arm64,amd64}/libfftw3f.a
...
PKGCONFIG_BASE="${REPO_ROOT}/internal/infra/audio/fftw3_libs/pkgconfig/darwin"
...
build_arch() {
    ...
    echo "==> Building FFTW3F ${FFTW_VERSION} static lib for darwin/${OUT_ARCH}..."
    rm -rf "${WORK_DIR}"
    ...
    ./configure --host="${HOST_TRIPLE}" --prefix="${INSTALL_DIR}" \
        ...
        --enable-single --disable-fortran --disable-doc >/dev/null
    make -j"$(sysctl -n hw.ncpu)" >/dev/null
    make install >/dev/null

    local LIB="${INSTALL_DIR}/lib/libfftw3f.a"
    if [[ ! -f "${LIB}" ]]; then
        echo "    ERROR: libfftw3f.a not produced" >&2
        exit 1
    fi

    mkdir -p "${OUT_BASE}/${OUT_ARCH}"
    cp "${LIB}" "${OUT_BASE}/${OUT_ARCH}/libfftw3f.a"
    echo "    copied libfftw3f.a -> fftw3_libs/darwin/${OUT_ARCH}/"

    verify_fftw3 "${OUT_BASE}/${OUT_ARCH}/libfftw3f.a"
...
verify_fftw3() {
    ...
    grep -q "_fftwf_plan_dft_1d" <<<"${SYMS}" || { echo "    ERROR: fftwf_plan_dft_1d missing from libfftw3f.a"; exit 1; }
