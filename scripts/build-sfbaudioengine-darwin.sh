...
build_arch() {
    ...
    local SCRATCH_PATH="${TMP_SRC}/.build-${ARCH}"
    local BUILD_OUTPUT="${SCRATCH_PATH}/${ARCH}-apple-macosx/release"
    local SWIFT_HEADER="${BUILD_OUTPUT}/SFBAudioEngine.build/include/SFBAudioEngine-Swift.h"
    ...
    local DEPS=("AVFAudioExtensions" "CXXAudioRingBuffer" "CXXDispatchSemaphore" "CXXMessageQueue" "CXXQueue" "CXXUnfairLock" "MAC")
    for dep in "${DEPS[@]}"; do
        echo "[sfb] Building dependency: ${dep}..."
        (cd "${TMP_SRC}" && swift build -c release --arch "${ARCH}" --scratch-path "${SCRATCH_PATH}" --target "${dep}" > /dev/null 2>&1 || true)
    done

    # Build main targets
    (cd "${TMP_SRC}" && swift build -c release --arch "${ARCH}" --scratch-path "${SCRATCH_PATH}" --target CSFBAudioEngine 2>&1 | grep -E "error:|Build complete" | tail -5 || true)
    (cd "${TMP_SRC}" && swift build -c release --arch "${ARCH}" --scratch-path "${SCRATCH_PATH}" --target SFBAudioEngine 2>&1 | grep -E "error:|Build complete" | tail -5 || true)

    # Swift 6's native build system stores products in out/ rather than the
    # target-triple directory used by older SwiftPM releases.
    if [ -d "${SCRATCH_PATH}/out/Products/Release" ]; then
        BUILD_OUTPUT="${SCRATCH_PATH}/out/Products/Release"
        SWIFT_HEADER="${SCRATCH_PATH}/out/Intermediates.noindex/SFBAudioEngine.build/Release/SFBAudioEngine-t.build/Objects-normal/${ARCH}/SFBAudioEngine-Swift.h"
    fi

    # Collect ALL object files generated by SPM (including dependencies like TagLib)
    local OBJ_FILES
    OBJ_FILES=$(find "${BUILD_OUTPUT}" -name "*.o" | tr '\n' ' ')
    ...
    if [ -d "${BUILD_OUTPUT}/Modules/SFBAudioEngine.swiftmodule" ]; then
        cp -R "${BUILD_OUTPUT}/Modules/SFBAudioEngine.swiftmodule/"* "${FW_DIR}/Versions/A/Modules/SFBAudioEngine.swiftmodule/"
    elif [ -d "${BUILD_OUTPUT}/SFBAudioEngine.swiftmodule" ]; then
        cp -R "${BUILD_OUTPUT}/SFBAudioEngine.swiftmodule/"* "${FW_DIR}/Versions/A/Modules/SFBAudioEngine.swiftmodule/"
    else
        # If it's a file, copy the relevant files into the directory and rename to architecture
        cp "${BUILD_OUTPUT}/Modules/SFBAudioEngine.swiftmodule" "${FW_DIR}/Versions/A/Modules/SFBAudioEngine.swiftmodule/${ARCH}.swiftmodule"
        [ -f "${BUILD_OUTPUT}/Modules/SFBAudioEngine.swiftdoc" ] && cp "${BUILD_OUTPUT}/Modules/SFBAudioEngine.swiftdoc" "${FW_DIR}/Versions/A/Modules/SFBAudioEngine.swiftmodule/${ARCH}.swiftdoc"
        [ -f "${BUILD_OUTPUT}/Modules/SFBAudioEngine.swiftsourceinfo" ] && cp "${BUILD_OUTPUT}/Modules/SFBAudioEngine.swiftsourceinfo" "${FW_DIR}/Versions/A/Modules/SFBAudioEngine.swiftmodule/${ARCH}.swiftsourceinfo"
        [ -f "${BUILD_OUTPUT}/Modules/SFBAudioEngine.abi.json" ] && cp "${BUILD_OUTPUT}/Modules/SFBAudioEngine.abi.json" "${FW_DIR}/Versions/A/Modules/SFBAudioEngine.swiftmodule/${ARCH}.abi.json"
    fi

    # Copy generated Swift header and C headers
    cp "${SWIFT_HEADER}" "${FW_DIR}/Versions/A/Headers/"
