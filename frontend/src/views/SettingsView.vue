...
<template>
  <div class="p-8 max-w-4xl mx-auto">
    ...
    <!-- Sync Progress UI -->
    <div v-if="isSyncing && syncProgress" class="mb-8 p-6 bg-card rounded-xl border border-primary/20 shadow-sm animate-in fade-in slide-in-from-top-2">
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-3">
          <Loader2 class="w-5 h-5 animate-spin text-primary" />
          <h2 class="font-semibold">Syncing Music Library...</h2>
        </div>
        <span class="text-sm font-medium text-muted-foreground bg-secondary px-2 py-1 rounded">
          {{ syncProgress.current }} / {{ syncProgress.total }}
        </span>
      </div>
      
      <div class="w-full bg-secondary rounded-full h-2 mb-3 overflow-hidden">
        <div 
    ...
    <div class="space-y-8">
      <section class="bg-card rounded-xl border p-6 shadow-sm">
        <div class="flex items-center justify-between mb-6">
          <div>
            ...
            <p class="text-sm text-muted-foreground">Manage the folders where Airmedy looks for music.</p>
          </div>
          <button 
            @click="addFolder"
            :disabled="isSyncing"
            class="flex items-center gap-2 px-3 py-1.5 bg-secondary text-secondary-foreground rounded-md hover:bg-secondary/80 transition-colors text-sm font-medium disabled:opacity-50"
          >
            <Plus class="w-4 h-4" />
            Add Folder
          </button>
        ...
        <div v-if="isLoading" class="py-12 flex justify-center">
          <RotateCcw class="w-8 h-8 animate-spin text-muted-foreground" />
        </div>
        
        <div v-else-if="folders.length === 0" class="py-12 text-center border-2 border-dashed rounded-lg">
          <Folder class="w-12 h-12 mx-auto text-muted-foreground mb-4 opacity-20" />
          <p class="text-muted-foreground">No music folders added yet.</p>
        ...
        <ul v-else class="divide-y border rounded-lg overflow-hidden bg-background/50">
          <li v-for="folder in folders" :key="folder.id" class="flex items-center justify-between p-4 hover:bg-accent/50 transition-colors">
            <div class="flex items-center gap-3 overflow-hidden">
              <Folder class="w-5 h-5 text-muted-foreground flex-shrink-0" />
              <span class="text-sm font-medium truncate" :title="folder.path">{{ folder.path }}</span>
            </div>
            <button 
              @click="removeFolder(folder.id)"
              :disabled="isSyncing"
              class="p-2 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-md transition-all disabled:opacity-50"
              title="Remove folder"
            >
      ...
      <section class="bg-card rounded-xl border p-6 shadow-sm opacity-50">
        <h2 class="text-xl font-semibold mb-1">Appearance</h2>
        <p class="text-sm text-muted-foreground mb-4">Theming and UI customizations coming soon.</p>
