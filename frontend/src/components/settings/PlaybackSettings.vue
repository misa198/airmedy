...
import { AudioLines, Wrench, Volume2 } from '@lucide/vue'
...
<template>
  <div class="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-500">
    <SettingSection :icon="Wrench" :label="t('settings.playback.general')">
      <SettingRow :title="t('settings.playback.prevent_sleep')" :description="t('settings.playback.prevent_sleep_desc')">
        <Switch
          :model-value="appStore.preventSleepWhilePlaying"
          @update:model-value="appStore.updatePreventSleepWhilePlaying"
        />
      </SettingRow>
      <SettingRow :title="t('settings.playback.show_player_indicator')" :description="t('settings.playback.show_player_indicator_desc')">
        <Switch
    ...
    <SettingSection :icon="Volume2" :label="t('settings.normalization.title')">
      ...
      <SettingRow
        ...
        :title="t('settings.normalization.enable')"
        :description="t('settings.normalization.enable_desc')"
      >
        <Switch
      ...
      <SettingRow
        class="transition-opacity"
        :class="(!appStore.libraryAnalysisEnabled || !appStore.normalizationEnabled) && 'opacity-40 pointer-events-none'"
        :title="t('settings.normalization.prevent_clip')"
        :description="t('settings.normalization.prevent_clip_desc')"
      >
        <Switch
