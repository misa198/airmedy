...
<template>
  <div
    class="inline-flex items-center p-1 rounded-full bg-black/20 backdrop-blur-md border border-white/5 relative h-10 select-none mix-blend-screen isolate shadow-2xl">
    <!-- Sliding background for active tab -->
    <div
      class="absolute top-1 bg-white rounded-full transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] shadow-[0_2px_8px_rgba(255,255,255,0.2)]"
      :style="sliderStyle" />
    ...
    <button v-for="option in props.options" :key="option.value" @click="handleClick(option.value)"
      ...
      :class="props.modelValue === option.value ? 'text-black/50' : 'text-white/40 hover:text-white/70'"
      :title="option.label">
