...
`applyPrimaryColor()` applies the saved normalized `#RRGGBB` accent to `--primary` and `--primary-rgb`, and chooses a black or white primary foreground for contrast. The `--primary-100` and `--primary-200` selected-row shades use opaque CSS `color-mix()` values derived from `--primary`: they blend toward white in light mode and toward a dark neutral in dark/black modes, so they update with the configured accent without transparency. General Settings offers seven circular presets (`#E11D48`, `#2563EB`, `#7E22CE`, `#DB2777`, `#EA580C`, `#CA8A04`, `#15803D`) plus a shared in-app color picker for any valid hex color. The horizontal preset group uses half the row width (capped at 18rem), so its controls wrap in a narrow Settings section; the row title truncates rather than wrapping.
...
- **Windows** (`service_windows.go`): the app installs under Program Files (admin), so a normal-user process cannot swap the running `.exe`. `applyUpdate` extracts the NSIS **installer** `.exe` from the release zip to a temp staging path; `relaunch` waits for exit then runs the installer **silently and elevated (UAC)** and relaunches the app. The NSIS template removes Airmedy Start Menu and desktop shortcuts from both the current-user and all-users shell contexts, including legacy root-level Start Menu entries, before recreating the canonical all-users Start Menu shortcut in the `Entertainment` folder. This keeps one shortcut after an elevated update and migrates existing installations into that folder.
