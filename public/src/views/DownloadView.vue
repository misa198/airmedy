...
type Platform = { name: string, icon: string, files: [string, string][] }
...
const platforms: Platform[] = [
  ...
  { name: 'Android', icon: '/android.svg', files: [] },
]

onMounted(async () => {
  try {
    version.value = (await fetch('/config.json').then(response => response.json())).downloadVersion
  } catch {
    version.value = ''
  }
})
          ...
          <div v-for="platform in platforms" :key="platform.name" class="download-group">
            ...
            <div v-if="platform.files.length" class="dropdown">
              ...
              <div class="dropdown-content"><div class="dropdown-content-inner glass">
                <a v-for="([label, file]) in platform.files" :key="file" :href="version ? `${releases}${file}` : undefined">{{ label }}</a>
              </div></div>
            </div>
