...
**Agent Response:**
- Perform research using `grep_search`, `read_file`, and other read-only tools.
...
If the intent is unclear, the agent must use the `ask_user` tool to clarify whether the request is an Inquiry or a Directive before taking any action that modifies the workspace.
