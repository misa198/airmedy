...
func TestProjectCleansShortcutsAcrossShellContexts(t *testing.T) {
	...
	cleanup := "Function RemoveAirmedyShortcuts"
	cleanupAt := strings.Index(source, cleanup)
	if cleanupAt < 0 {
		t.Fatal("installer must define shared shortcut cleanup")
	}
	cleanupEnd := strings.Index(source[cleanupAt:], "FunctionEnd")
	if cleanupEnd < 0 {
	...
	installCleanup := strings.Index(source, "Call RemoveAirmedyShortcuts")
	...
	if strings.Count(source, "Call RemoveAirmedyShortcuts") != 2 {
