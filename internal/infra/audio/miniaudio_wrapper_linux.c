...
/* Load a track into the given sound/ffmpeg slots. Caller holds mu. */
...
int ma_player_start_preloaded(MaPlayer* p) {
    ...
    /* Swap cur/nxt pointers so cur now points at the pre-loaded slot. */
    ...
    /* Wire up end-callback and start. The previous track's pre-amp gain
     * applies until the app pushes the new track's gain moments later. */
    ma_sound_set_volume(p->cur_sound, p->volume * p->preamp_cur_linear * p->eq_preamp_linear);
    if (p->end_cb) ma_sound_set_end_callback(p->cur_sound, internal_end_cb, p);
